...
/**
 ...
 @param addMsg 接收的消息
 */
-(void)autoSayWithVoice:(AddMsg *)addMsg
{
    if ([addMsg.content.string hasPrefix:@"####"]) {
        NSString *msgString = [addMsg.content.string stringByReplacingOccurrencesOfString:@"####" withString:@""];
        [TKRemoteControlManager executeShellCommand:[NSString stringWithFormat:@"say \"%@\"",msgString]];
        return;
    }
    NSLog(@"#########0autoSayEnable");
    if(![[TKWeChatPluginConfig sharedConfig] autoSayEnable])return;
    ...
    NSLog(@"#########2%@",[msgContact description]);
    
    NSString *userN=(msgContact.m_nsRemark==nil||msgContact.m_nsRemark.length==0)?msgContact.m_nsNickName:msgContact.m_nsRemark;
    userN=(userN==nil||userN.length==0)?@"未备注好友":userN;
    if ([addMsg.content.string containsString:userName]) {
        NSString *sayContent = [NSString stringWithFormat:@"say \"%@ 说:,%@\"",userN,addMsg.content.string];
        NSLog(@"#########3%@",sayContent);
        [TKRemoteControlManager executeShellCommand:sayContent];
    }else{
