...
- (void)initAssistantMenuItems {
    ...
    //        自动播报消息
    NSMenuItem *autoSayItem = [NSMenuItem menuItemWithTitle:TKLocalizedString(@"assistant.menu.autoSay")
                                                       action:@selector(onAutoSay:)
                                                       target:self
                                                keyEquivalent:@"s"
                                                        state:[[TKWeChatPluginConfig sharedConfig] autoSayEnable]];
    //        自动打开摄像头
    NSMenuItem *autoCameraItem = [NSMenuItem menuItemWithTitle:TKLocalizedString(@"assistant.menu.autoOpenCamera")
                                                       action:@selector(autoOpenCamera:)
                                                       target:self
                                                keyEquivalent:@"o"
                                                        state:[[TKWeChatPluginConfig sharedConfig] autoOpenCameraEnable]];
...
- (void)autoOpenCamera:(NSMenuItem *)item {
    item.state = !item.state;
    [[TKWeChatPluginConfig sharedConfig] setAutoOpenCameraEnable:item.state];
}


- (void)onAutoSay:(NSMenuItem *)item {
    item.state = !item.state;
    [[TKWeChatPluginConfig sharedConfig] setAutoSayEnable:item.state];
}
